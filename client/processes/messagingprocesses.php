...
elseif(isset($_POST["chatarea"])){
    ...
    $qry = "SELECT * FROM project_messages JOIN client_case ON client_case.case_id=project_messages.project_id JOIN client ON client.client_id=client_case.client_id JOIN lawyer_case ON lawyer_case.case_id=client_case.case_id JOIN lawyer ON lawyer.lawyer_id=lawyer_case.lawyer_id WHERE project_messages.project_id = '$orid' ORDER BY proj_msg_id ASC";
            ...
            if($row["msg_type"] == 1){
                if($refrow["ref_type"] == 0){
                    $info_ref = '<div class="m-1 alert alert-info shadow">
                                    <span class="text-sm">@about: <a href="submissions.php?job='.$row["job_id"].'">Order Progress/Milestone</a></span>
                                </div>';    
                }elseif($refrow["ref_type"] == 1){
                    $info_ref = '<div class="m-1 alert alert-info shadow">
                                    <span class="text-sm">@about: <a href="submissions.php?job='.$row["job_id"].'#final" target=_blank>Final Submission</a></span>
                                </div>';  
                }elseif($refrow["ref_type"] == 2){
                    $info_ref = '<div class="m-1 alert alert-info shadow">
                                   <span class="text-sm">@about: <a href="submissions.php?job='.$row["job_id"].'#revision" target=_blank>Order Revision</a></span>
                                </div>';  
                }
            }elseif($row["msg_type"] == 0){
                if($refrow["ref_type"] == 0){
                    $grey_ref = '<div class="m-1 alert alert-grey shadow">
                                    <span class="text-sm">@about: <a href="submissions.php?job='.$row["job_id"].'" target=_blank>Progress submission</a></span>
                                </div>'; 
                }elseif($refrow["ref_type"] == 1){
                    $$grey_ref = '<div class="m-1 alert alert-grey shadow">
                                    <span class="text-sm">@about: <a href="submissions.php?job='.$row["job_id"].'#final" target=_blank>Order Submission</a></span>
                                </div>';  
                }elseif($refrow["ref_type"] == 2){
                    $$grey_ref = '<div class="m-1 alert alert-grey shadow">
                                   <span class="text-sm">@about: <a href="submissions.php?job='.$row["job_id"].'#revision" target=_blank>Order Revision</a></span>
...
elseif(isset($_POST["unreadstatus"])){
    ...
    $qry = "UPDATE project_messages SET msg_status=1 WHERE project_id='$orid' AND msg_status=0 AND msg_type=0";
...
elseif(isset($_POST["reloadChats"])){
    ...
    $res = $conn->query("SELECT *, COUNT(case when project_messages.msg_status=0 AND project_messages.msg_type=0 then  project_messages.msg_status end) AS unreadmessages FROM project_messages JOIN client_case ON client_case.case_id=project_messages.project_id  WHERE client_case.client_id='$studID' AND project_messages.date_sent >= '$date_ago' AND project_messages.msg_status=0 AND project_messages.msg_type=0 GROUP BY project_messages.project_id ORDER BY MAX(project_messages.proj_msg_id) ");
    if($res->num_rows > 0){
        while($row = $res->fetch_assoc()){
            $ord = $row["project_id"];
            $last_sms_qry = "SELECT * FROM project_messages WHERE project_id='$ord' AND msg_type=0 ORDER BY proj_msg_id DESC LIMIT 1";
...
elseif(isset($_POST["reloadByOrid"])){
    ...
    $qry = "SELECT * FROM project_messages JOIN client_case ON client_case.case_id=project_messages.project_id JOIN lawyer_case ON lawyer_case.case_id=client_case.case_id JOIN lawyer ON lawyer.lawyer_id=lawyer_case.lawyer_id WHERE project_messages.project_id = '$orid' AND project_messages.date_sent >= '$date_ago' AND project_messages.msg_type=0 ORDER BY proj_msg_id ASC";
    ...
    while($row = $res->fetch_assoc()){
        ...
        if($row["ref_id"] != 0){
            ...
            if($row["msg_type"] == 1){
                if($refrow["ref_type"] == 0){
                    $info_ref = '<div class="m-1 alert alert-info shadow">
                                    <span class="text-sm">@about: <a href="submissions.php?job='.$row["job_id"].'" target=_blank >Order Progress/Milestone</a></span>
                                </div>';    
                }elseif($refrow["ref_type"] == 1){
                    $info_ref = '<div class="m-1 alert alert-info shadow">
                                    <span class="text-sm">@about: <a href="submissions.php?job='.$row["job_id"].'#final" target=_blank>Final Submission</a></span>
    ...
    $uqry = "UPDATE project_messages SET msg_status=1 WHERE project_id='$orid' AND msg_status=0 AND msg_type=0";
